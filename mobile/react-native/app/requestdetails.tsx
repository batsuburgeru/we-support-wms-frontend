  ...
  export default function RequestDetails(): JSX.Element {
    ...
    useEffect(() => {
      const fetchRequestDetails = async () => {
        try {
          setLoading(true);
          const API_URL = `http://172.16.48.90:3002/purchaseRequests/read-purchase-requests`;
    ...
    const handleConfirm = async () => {
      ...
      try {
        const API_URL = `http://172.16.48.90:3002/purchaseRequests/update-purchase-request-status/${request.id}`;
        const payload = {
          status:
            modalType === 'Approve'
              ? 'Approved'
